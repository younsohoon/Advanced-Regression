...
kfold = function(n){
  ...
  # set.seed(777) 
  # fit = gam(los ~ s(weight) + s(height) + s(heartrate_mean) + ti(weight, height),
  #          data = dat2) # 20.85729 ss, dat2
  # set.seed(777)
  # fit = randomForest(los ~ weight + height + heartrate_mean + aniongap + admission_location,
  #                    mtry = 3, ntree=1000, maxnodes= 300,
  #                    nodesize = 20, sampsize = 350,
  #                    data = dat, importance=TRUE) # 16.111, dat
  # set.seed(777)
  # fit = randomForest(los ~ weight + height + heartrate_mean + aniongap + admission_location,
  #                    mtry = 3, ntree=1000, maxnodes= 300,
  #                    nodesize = 20, sampsize = 350,
  #                    data = dat2, importance=TRUE) # 15.4598 rf, dat2
...
kfold = function(n){
  ...
  set.seed(777) 
  m1 = lm((los)^(1/2)~weight+height+weight*height+heartrate_mean,data=dat) # dat 18.92399
  # m1 = lm((los)^(1/2)~weight+height+weight*height+heartrate_mean,data=dat2) # dat2 18.88445
